...
function oop_link_to_modul($modul, $seite = 'main', $titelzeile_modul = '')
{
	...
	$module_information = get_module_information($titelzeile_modul);
...
class WebBase_Module_Info
{
	...
	var $rights;
	var $no_security; // NUR BEI *_AUSLOGGEN VERWENDEN!!!
	var $menu_visible;
...
{
	global $cache_module_information;
	if ((isset($cache_module_information)) && (isset($cache_module_information[$modulename]))) {
		return $cache_module_information[$modulename];
	}
	...
	if ((strpos($modulename, '..') === false) && (file_exists(__DIR__ . '/../modules/'.$modulename.'/info.xml')))
	{
		$object = $xml->xml_file_to_object(__DIR__ . '/../modules/'.$modulename.'/info.xml');

		if ($object->name == 'moduleinfo')
		{
			$output = new WebBase_Module_Info;

			foreach ($object->children as $m1 => $m2)
			{
				if ($object->children[$m1]->name == 'caption')				$output->caption			= $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'primary_position')		$output->primary_position	= $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'secondary_position')	$output->secondary_position	= $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'rights')				$output->rights				= $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'no_security')			$output->no_security		= $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'menu_visible')			$output->menu_visible		= $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'author')				$output->author				= $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'version')				$output->version			= $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'language')				$output->language			= $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'license')				$output->license			= $object->children[$m1]->content;
			}
			...
			$cache_module_information[$modulename] = $output;
...
{
	global $cache_design_information;
	if ((isset($cache_design_information)) && (isset($cache_design_information[$designname]))) {
		return $cache_design_information[$designname];
	}
	...
	if ((!strpos($designname, '..')) && (file_exists('designs/'.$designname.'/info.xml')))
	{
		...
		{
			$output = new WebBase_Design_Info;

			foreach ($object->children as $m1 => $m2)
			{
				if ($object->children[$m1]->name == 'name') $output->name = $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'author') $output->author = $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'version') $output->version = $object->children[$m1]->content;
				if ($object->children[$m1]->name == 'license') $output->license = $object->children[$m1]->content;
			}
			...
			$cache_design_information[$designname] = $output;
