...
==========================================================================*/
	...
	if (isset($GLOBALS[action]) && ($GLOBALS[action] == "reply" || $GLOBALS[action] == "replytoall")) {
		...
		$query = "UPDATE webbase_popper_messages SET was_read='$read_status' WHERE id='$this->cur_message_id' AND konto='$this->userid'";
		$res = $this->db_tool->db_query($query);
	...
	if (!$readonly) {
		$query = "SELECT replyaddr FROM webbase_popper_konten WHERE id='$this->userid'";
...
<?php
		...
		$res = mysql_query("SELECT email, name FROM webbase_kontakte WHERE email != '' ORDER BY id;");
