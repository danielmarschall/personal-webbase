...
class mailer extends mail {
	...
	function load($id) {
		$query = "SELECT * FROM webbase_popper_messages WHERE id = '$id' AND konto='$this->user_id'";
	...
	function store($folder, $msg = false, $server_uidl = 0) {
		...
		global $webbase_userid;
		$query = "INSERT INTO webbase_popper_messages (user, tos, froms, from_name, subject, date, dir, konto, mail, uidl, is_mime, was_read, priority) VALUES ('$webbase_userid', '$tos', '$froms', '$from_name', '$subject', $date, '$folder', '$this->user_id', '$msg', '$uidl', '$is_mime', '$was_read', '$priority')";
		$res = $this->db_tool->db_query($query);
