	...
	$my_str = substr($my_str, 0, strlen($my_str)-2);
		...
		foreach ($modules as $m1 => $m2)
		{
			if (file_exists('modules/'.$m2.'/page/main.inc.php'))
			{
				...
				{
					$caption = $module_information->caption;
					$found_stuff[$caption][] = array($module_information, $m2);
				}
			}
		}
		...
		$i = -1;
		foreach ($found_stuff as $tmp) {
			foreach ($tmp as $data) {
				$module_information = $data[0];
				$m2 = $data[1];
				...
				echo '<br>'.wb_htmlentities($module_information->caption).'</a></td>';
			}
		}
