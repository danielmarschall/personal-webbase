...
if (($aktion == 'new') || ($aktion == 'edit'))
{
  ...
  if ($aktion == 'edit')
  {
    $res = db_query("SELECT * FROM `".$mysql_zugangsdaten['praefix']."net2ftp` WHERE `id` = '".db_escape($id)."' AND `user` = '".$benutzer['id']."'");
    if (db_num($res) == 0)
    ...
    $fstart = (isset($row['startverzeichnis'])) ? $row['startverzeichnis'] : '';
...
<tr>
  <td valign="top">Startverzeichnis:<img src="design/spacer.gif" height="1" width="35" alt=""></td>
