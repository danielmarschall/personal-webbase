...
if (($aktion == 'new') || ($aktion == 'edit'))
{
  ...
  if ($aktion == 'edit')
  {
    $res = db_query("SELECT `name`, `folder` FROM `".$mysql_zugangsdaten['praefix']."ordner` WHERE `id` = '".db_escape($id)."' AND `user` = '".$benutzer['id']."'");
    if (db_num($res) == 0)
...
<tr>
  <td valign="top">Kategorie:<img src="design/spacer.gif" height="1" width="35" alt=""></td>
  <td valign="top">'.$modulueberschrift.'</td>
</tr>
<tr>
...
<tr>
  ...
  if (!isset($id)) $id = 0;
  gfx_zeichneordnerbox($kategorie, 'ORDER BY `name`', 0, 0, $auszuwaehlen, $id);
