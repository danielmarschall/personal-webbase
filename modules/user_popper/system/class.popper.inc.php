...
class popper {
	...
	function get_userid($name = "") {
		...
		$query = "SELECT id FROM ironbase_popper_konten WHERE STRCMP(name, '$name') = 0";
		$res = $this->db_tool->db_query($query);
		if ($res == 0) {
	...
	function show_notification_request() {
		...
		if (!empty($notify)) {
			$query = "SELECT was_read FROM ironbase_popper_messages WHERE konto = '$this->userid' AND id='$this->cur_message_id'";
			$res = $this->db_tool->db_query($query);
	...
	function show_folders() {
		$unread = MAIL_UNREAD;
		$query = "SELECT dir, count(*) as count FROM ironbase_popper_messages WHERE was_read='$unread' AND konto='$this->userid' GROUP BY dir";
		$res = $this->db_tool->db_query($query);
		if ($res != 0) {
			while($row = $this->db_tool->fetch_array($res)) {
	...
	function count_mails() {
		...
		$query = "SELECT COUNT(*) FROM ironbase_popper_messages WHERE konto='$this->userid' AND dir='$this->folder'";
		$res = $this->db_tool->db_query($query);
		if ($res == 0) {
			$this->report_error($this->db_tool->db_error(), $strings["l_DBError"]);
			return;
	...
	function list_mails() {
		...
		while(1) {
			...
			$query = "SELECT id, tos, from_name as froms, subject, is_mime, date, was_read, priority FROM ironbase_popper_messages WHERE dir = '$this->folder' AND konto = '$this->userid' ORDER BY $order";
...
<?php
	...
	function show_mail_content($mailer = 0) {
		...
		if ($action == "showsources") {
			if ($mail_id != "att") {
				$query = "SELECT mail FROM ironbase_popper_messages WHERE id='$mail_id' AND konto='$this->userid'";
				$res = $this->db_tool->db_query($query);
				if ($res == 0) {
	...
	function next_prev() {
		...
		// Get all mail ids
		$query = "SELECT id FROM ironbase_popper_messages WHERE konto='$this->userid' ORDER BY $this->order";
		$res = $this->db_tool->db_query($query);
		if ($res == 0) {
			$this->report_error($this->db_tool->db_error(), $strings["l_DBError"]);
			exit;
...
<?php
	...
	function set_mail_read($mail_id) {
		$query = "UPDATE ironbase_popper_messages SET was_read='".MAIL_READ."' WHERE id='$mail_id' AND konto='$this->userid' AND was_read='".MAIL_UNREAD."'";
	...
	function store_mail($backup = false) {
		...
		$query = "SELECT personenname, replyaddr FROM ironbase_popper_konten WHERE id='$this->userid'";
		$res = $this->db_tool->db_query($query);
		if ($res == 0) {
	...
	function add_sel_addr($block = false) {
		...
		if (is_array($del)) {
			$query = "SELECT froms FROM ironbase_popper_messages WHERE id IN (";
			foreach($del as $in) {
				$query .= $in.", ";
			}
	...
	function delete_all() {
		...
		$query = "SELECT id FROM ironbase_popper_messages WHERE konto='$this->userid' AND dir='$this->folder'";
		$res = $this->db_tool->db_query($query);
		if ($res == 0) {
	...
	function move_to_bin($id) {
		if (is_array($id)) {
			$query = "UPDATE ironbase_popper_messages SET dir='bin' WHERE id IN (";
			foreach($id as $in) {
				$query .= $in.", ";
			}
			// Remove the last comma
			...
			$query = "UPDATE ironbase_popper_messages SET dir='bin' WHERE id='$id' AND konto='$this->userid'";
		}
		$res = $this->db_tool->db_query($query);
		if ($res == 0) {
			return false;
	...
	function trash_mail($id) {
		...
		$query = "SELECT ironbase_popper_konten.*, ironbase_popper_messages.uidl AS uidl FROM ironbase_popper_konten LEFT JOIN ironbase_popper_messages ON ironbase_popper_messages.konto = ironbase_popper_konten.id WHERE ironbase_popper_messages.id IN (";
		foreach($id as $in) {
			$query .= $in.", ";
		}
		// Remove the last comma
		$query = substr($query, 0, strlen($query) - 2);
		$query .= ") AND ironbase_popper_messages.konto = '$this->userid' ORDER BY ironbase_popper_konten.id";
		$res = $this->db_tool->db_query($query);
		...
		if ($res != 0) {
			...
			while($this->config["server_del"] == 1 && $row = $this->db_tool->fetch_array($res)) {
				...
				if ($row[id] != $cur_account) {
					...
					$count = $pop3->pop_login($row["username"], $row["passwort"]);
					$cur_account = $row[id];
		...
		$query = "DELETE FROM ironbase_popper_messages WHERE id IN (";
		foreach($id as $in) {
			$query .= $in.", ";
		}
		// Remove the last comma
	...
	function show_del_mails() {
		...
		$query = "SELECT id, tos, froms, subject, date FROM ironbase_popper_messages WHERE dir='$this->folder' AND konto='$this->userid'";
		$res = $this->db_tool->db_query($query);
		if ($res == 0) {
...
<?php
	...
	function get_mails() {
		...
		$query = "SELECT uidl FROM ironbase_popper_messages WHERE konto = '$this->userid' AND dir != 'outbox' AND dir != 'sent'";
		$res = $this->db_tool->db_query($query);
		...
		$query = "SELECT * FROM ironbase_popper_konten WHERE id = '$this->userid'";
...
if (true) {
			...
			foreach($get as $num => $uid) {
				...
				if ($own_uidl) {
					...
					$query = "UPDATE ironbase_popper_messages SET uidl='".$server_uidls[$num]."' WHERE konto = '$this->userid' AND id = '$insert_id'";
	...
	function send_outbox() {
		...
		$query = "SELECT id FROM ironbase_popper_messages WHERE konto='$this->userid' AND dir='outbox'";
		...
		while($row = $this->db_tool->fetch_array($res)) {
			...
			if ($ret) {
				$send_counter++;
				$query = "UPDATE ironbase_popper_messages SET dir='sent', was_read = ".MAIL_READ." WHERE id='$row[id]' AND konto='$this->userid'";
				$update_res = $this->db_tool->db_query($query);
				if ($update_res == 0) {
